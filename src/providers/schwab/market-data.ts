...
export class SchwabMarketDataProvider implements MarketDataProvider {
   ...
   * were requested - see normalizeSchwabQuotesResponse). Each chunk's failure is isolated: a
   * single bad/oversized/rejected chunk never prevents the other chunks' symbols from coming
   * back - their entries are simply absent from the result Map, exactly like an individual
   * invalid symbol would be, never a thrown error for the whole batch. Deterministic:
   * getQuotes(["a","b"]) always maps to exactly {A: quoteForA, B: quoteForB} minus whichever
   * symbols were genuinely unavailable - never a cross-symbol mixup between chunks.
   */
  async getQuotes(symbols: string[]): Promise<Map<string, MarketQuote>> {
    ...
    for (const batch of chunks) {
      try {
        ...
        for (const [symbol, quote] of normalizeSchwabQuotesResponse(batch, payload)) {
