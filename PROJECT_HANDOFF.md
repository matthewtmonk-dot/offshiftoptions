...
**`prisma/migrations/20260831211006_account_ledger_and_broker_link/` is applied to local Docker Postgres ONLY — NOT yet applied to production Supabase.** It is purely additive (two new enums `AccountLedgerEntryType`/`AccountSource`, a new `AccountLedgerEntry` table, three new nullable columns on `TradingAccount` — `source`, `brokerConnectionId`, `externalAccountId` — one default-value change on `TradingAccount.visibility` from `SHARED` to `PRIVATE` for new rows only, a new unique index, and two new FKs). **Do not push/deploy the code that depends on this migration (Account Ledger, Tracker Performance/Accounts tabs, Schwab account sync) until this migration has been applied to Supabase and verified**, per the workflow above. See Current State for the exact pending step.
...
| Vitest unit tests | `corepack pnpm test` | 65 passed / 19 skipped (DB tests skip without `RUN_DB_TESTS`) |
| Vitest incl. DB integration | `RUN_DB_TESTS=1 DATABASE_URL=<local Docker Postgres URL> corepack pnpm test` | 89/89 passed across 21 files against local Docker Postgres |
...
- `prisma/migrations/` — migrations: `20260828114500_init`, `20260828132500_phase_1b_hardening`, `20260828215700_campaign_tracker_foundation` (all three applied to production Supabase), and `20260831211006_account_ledger_and_broker_link` (**local Docker only — not yet applied to production**, see Database / Seed / Bootstrap Safety).
...
- **Most recent completed work (this session, 2026-08-31): the "Core Trading Cockpit + Account/Tracking Usability" slice**, driven by an independent 53-finding UX audit of production (`OFF_SHIFT_OPTIONS_UX_AUDIT.md`, repo root) and taking over an interrupted Codex slice that had partially added the `AccountLedgerEntry` schema and LST Core threshold values. Delivered: the Account Ledger (schema + domain logic + `/account` UI), the LST Core scanner baseline with a "Reset to LST Core" action and diff badges (plus a real seed-data bug fix so fresh profiles actually match LST Core), gating-vs-preference honest score/label semantics on the scanner, a Schwab account-sync workflow with an honest Sync-now-first `/account` UI and a read-only Tracker open-positions panel, a rebuilt one-screen Dashboard cockpit, and the Tracker's Open/History/Performance/Accounts tabs with performance math that is provably never blended across users. Full detail is in the Account Ledger, Scanner, Schwab API, and Campaign Tracker sections above.
- **PENDING BEFORE PUSH: migration `20260831211006_account_ledger_and_broker_link` has NOT been applied to production Supabase.** It has been applied and verified against local Docker Postgres only. Do not push/deploy this session's code to `main` until: (1) Matt (or someone with the Supabase connection string) runs `DATABASE_URL="<supabase-connection-string>" pnpm prisma migrate deploy` against production and confirms with `prisma migrate status`, exactly as done for the two prior production migrations, then (2) this code is pushed. All local validation (typecheck, lint, unit + DB integration tests, Playwright against the rebuilt Docker app, production build) has passed — see Testing section — but nothing from this slice has been pushed or deployed yet.
- **Deferred from the UX audit, intentionally not attempted this slice** (engineering judgment — see the audit file for full detail): a full `/t/RIOT`-style Research page, a whole-options-universe scanner, whole-chain strike optimization, third-party earnings-calendar ingestion (the `earningsDistance` criterion stays honestly `UNKNOWN` on live Schwab scans — see Scanner section), achievements/gamification, a projections/forecasting engine, CSV import of historical trades, consolidating Recs/Chat/Alerts into one inbox, and a full navigation redesign (only the one low-risk, directly-tied rename — "Settings" → "Scanner Rules" — was made). These remain good candidates for future slices once the current cockpit/ledger foundation is live in production.
- **Nothing is currently blocked by Schwab approval or production env var setup.** The current Schwab production readiness step is: apply the pending migration, deploy, complete a real Schwab OAuth connection, run "Sync now," then run the first controlled live scan.
...
- Production Supabase already has the matching schema for everything through the Campaign Tracker migration.
- **This session's work (Account Ledger, LST Core, honest scanner semantics, Schwab account sync, Dashboard/Tracker rewrite) is validated locally but NOT committed/pushed, per explicit instruction, because its migration has not yet been applied to production.** Once the migration is applied to Supabase and confirmed, the next steps are: commit, push to `main`, let Hostinger auto-deploy (`prisma generate && next build` — no migration step, matching the existing deployment model), then verify `/api/health`, log in as both users, and spot-check `/dashboard`, `/positions` (all four tabs), `/account`, and `/scanner/settings`.

---

## Next Tasks

**NOW** (blocking this session's work)
- **Apply `prisma/migrations/20260831211006_account_ledger_and_broker_link` to production Supabase** the same way the two prior production migrations were applied: `DATABASE_URL="<supabase-connection-string>" pnpm prisma migrate deploy`, then confirm with `pnpm prisma migrate status`. Then commit and push this session's work (Account Ledger, LST Core scanner baseline, honest scanner semantics, Schwab account sync, Dashboard/Tracker rewrite) to `main` and let Hostinger auto-deploy.
- After that deploy: `/api/health`, log in as both Matt and Eric, spot-check `/dashboard` (no "No data" cards, weekly-return strip), `/positions` all four tabs, `/account` (ledger deposit/withdrawal form, Sync now if Schwab is connected), and `/scanner/settings` ("This profile matches LST Core" for either user who hasn't customized it).

**NOW** (no external blocker, independent of the above)
...
- Once deployed, run the production Schwab smoke test: `/account` Schwab connection panel → connect → "Sync now" → confirm account value/cash appear and the Tracker Open tab shows Schwab positions → `/scanner` "Run live Schwab scan" → confirm the latest run is labeled `LIVE • SCHWAB` rather than `DEMO`.
...
- Broker-to-campaign reconciliation UI (turning a Schwab "Possible match" into an actual explicit link) — deliberately deferred, see Schwab API / Campaign Tracker sections.
