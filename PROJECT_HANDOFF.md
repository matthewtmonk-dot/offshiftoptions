...
| Appearance (Dark/Light/System) | LOCAL SLICE COMPLETE, NOT DEPLOYED | Per-user Dark/Light/System theme, persisted on `UserSettings.appearance`; see Appearance System section. Requires migration `20260901031101_user_appearance_preference`, applied locally only. |
| Schwab market/account data + CSV import/reconciliation | LOCAL SLICE COMPLETE, NOT DEPLOYED | Production currently has the previously deployed read-only Schwab OAuth/account-sync foundation. The current local slice adds encrypted per-user Schwab developer app credentials, user-scoped market-data/personal-broker provider resolvers, provider-level market-data caching, a Schwab CSV import pipeline (positions/transactions/realized gain-loss, verified against real exports) with preview-before-persist and NEW/DUPLICATE/CONFLICT/NEEDS_REVIEW/INVALID classification, and a "Broker Activity Awaiting Review" reconciliation UI that links a broker position to a Campaign so the Dashboard counts it once. Verified end-to-end in a browser (upload -> preview -> confirm -> reconcile -> linked Campaign). This slice requires migration `20260831235050_schwab_credential_routing_broker_records`, which is applied locally only and must be applied to production Supabase before any push/deploy. No trading/order-entry functionality exists. |
...
- **Current status: production has the prior read-only Schwab OAuth/account-sync foundation live; the current per-user credential-routing/CSV normalization slice is implemented locally and not pushed/deployed because it requires migration `20260831235050_schwab_credential_routing_broker_records`, which is local-only until Matt applies it to Supabase.**
...
- Schema for this slice: `SchwabDeveloperCredential`, `BrokerConnection.developerCredentialId`, `BrokerRecord` (now with `status`, `identityKey`, `importBatchId`, `linkedCampaignId`, `reconciliationDismissedAt`), and `BrokerImportBatch` are all in the single migration `20260831235050_schwab_credential_routing_broker_records` (local Docker applied; production Supabase pending). This migration was regenerated/consolidated twice during this slice (each time via a local-only `prisma migrate reset`, run only with the user's explicit per-invocation consent per Prisma's own AI-safety guard) so the whole not-yet-deployed feature stays one coherent migration rather than several partial ones. Do not deploy dependent code before production migration is applied and verified.
...
Last verified (September 1, 2026, against local Docker PostgreSQL and a fresh local Next dev server on port 3001 — nothing run against Supabase):
...
| Vitest unit tests | `corepack pnpm test` | 113 passed / 45 skipped (DB tests skip without `RUN_DB_TESTS`) |
| Vitest incl. DB integration | `RUN_DB_TESTS=1 DATABASE_URL=<local Docker Postgres URL> corepack pnpm test` | 158/158 passed across 31 files against local Docker Postgres; existing `pg` deprecation warning only |
| Playwright e2e (scanner redesign), dev mode | `PLAYWRIGHT_BASE_URL=http://localhost:3001 corepack pnpm exec playwright test tests/e2e/scanner.spec.ts` against `next dev` | 12/12 passed |
| Playwright e2e (existing regression), dev mode | same, against `next dev` | 4/6 passed — 2 failures from the dev-only overlay artifact below |
| Playwright e2e, **production mode** (`next start`, local Docker Postgres, not Supabase) | `corepack pnpm build && corepack pnpm exec next start -p 3001`, then both spec files with `PLAYWRIGHT_BASE_URL=http://localhost:3001` | **phase1b.spec.ts 6/6 passed; scanner.spec.ts 12/12 passed** — confirms the dev-mode phase1b failures really were the dev-overlay artifact (see Known Issues) and not a real regression: the exact same Sign Out flow that timed out under `next dev` passes cleanly under `next start`, where no dev overlay exists. |

**Playwright login helper note**: both spec files originally logged in via the dev-only "Login as Matt/Eric" shortcut buttons (`login/page.tsx` renders them only when `NODE_ENV !== "production"`), which made it impossible to run either suite against `next start` at all — every test would hang waiting for a button that production mode never renders. Both `loginAs`/`loginAsMatt` helpers now fill the real email/password sign-in form (`matt@lst.local`/`eric@lst.local`, `DEV_SEED_PASSWORD` or its `lstbuddy-dev-only` default) instead, which works identically in both dev and production mode. No application code changed for this — test-helper only.
| Playwright e2e (appearance visual QA) | temporary `tests/e2e/zz-theme.spec.ts` against `PLAYWRIGHT_BASE_URL=http://localhost:3001` | 6/6 passed; screenshots viewed for Dark/Light/System across Dashboard, Tracker, Scanner, Scanner Rules, Watchlist, Account, Login (desktop + mobile), then the test file and screenshots were deleted per policy — never committed. |
...
- `prisma/migrations/` — migrations: `20260828114500_init`, `20260828132500_phase_1b_hardening`, `20260828215700_campaign_tracker_foundation`, `20260831211006_account_ledger_and_broker_link` are applied to production Supabase; `20260831235050_schwab_credential_routing_broker_records` (`SchwabDeveloperCredential`, `BrokerConnection.developerCredentialId`, `BrokerRecord`, `BrokerImportBatch`) and `20260901031101_user_appearance_preference` (`UserSettings.appearance`) are applied locally only and must be applied to production before their respective slices are pushed.
...
- **Scanner UX redesign for compact/high-density scanning is complete and locally verified (2026-09-01), not yet pushed/deployed.** See Scanner section for full detail. No schema migration required — view-layer only.
- **Multi-user Schwab credential routing + CSV import/reconciliation foundation is complete and locally verified (2026-08-31), not yet pushed/deployed.** Implemented: encrypted per-user Schwab developer credentials with OAuth resolution tied to the selected credential; user-scoped market-data (`resolveMarketDataProviderForUser`) and personal-broker (`resolvePersonalBrokerProviderForUser`) provider resolvers with no cross-user fallback (tested); provider-level market-data cache/in-flight coalescing; a Schwab CSV import pipeline for all three real export types (Positions/Transactions/Realized Gain-Loss - parser verified against real files Matt provided directly, not committed to Git) with a strict preview-before-persist lifecycle, NEW/DUPLICATE/CONFLICT/NEEDS_REVIEW/INVALID classification, and database-backed idempotency; and a "Broker Activity Awaiting Review" reconciliation UI that confirms broker evidence into a real Campaign (via the existing `createCampaignForUser` workflow) and durably links it so the Dashboard counts it once, not twice. Verified end-to-end in a real browser (upload -> preview -> confirm -> reconcile -> linked Campaign, with screenshots). This work is not pushed/deployed because migration `20260831235050_schwab_credential_routing_broker_records` must be applied to production Supabase first.

### Git / Deployment State (as of this session)

- The scanner enhancement work, the Campaign Tracker foundation, the Schwab OAuth/market-data foundation, the Core Trading Cockpit + Account Ledger slice, the Dashboard broker-position follow-up fix, and the Appearance System (Dark/Light/System) are **committed and pushed to `origin/main`**; production was previously live at commit `c6d7ddd`.
- The current working tree contains the local Schwab credential-routing/CSV-import/reconciliation slice (migration `20260831235050_schwab_credential_routing_broker_records`) **and** the Scanner UX redesign (no migration — view-layer only). Do not push either until reviewed/approved. **Do not push the Scanner UX redesign until Matt explicitly approves — it ends this session with a report only, per explicit instruction.**
...
**NOW**
- Matt to review and approve the Scanner UX redesign (report delivered at end of that session); no migration to apply — push whenever approved.
- Apply migration `20260831235050_schwab_credential_routing_broker_records` to production Supabase from a machine that can run Prisma's schema engine: `DATABASE_URL="<supabase-connection-string>" corepack pnpm prisma migrate deploy`. Then verify with `DATABASE_URL="<supabase-connection-string>" corepack pnpm prisma migrate status`. Do not paste or commit the Supabase URL.
- After Supabase confirms the migration is up to date, review the local diff, run final validation if anything changed, commit the Schwab credential-routing/CSV-import/reconciliation slice, and push to `origin/main` so Hostinger auto-deploys.
- Post-deploy smoke test: log in as Matt, visit `/account`, confirm Brokerage Connections -> Schwab shows the developer app panel without exposing secrets, confirm existing Schwab connection/sync still works; visit `/positions?view=accounts`, upload a real Schwab CSV export, confirm the preview counts look right, confirm the import, and confirm/skip anything in "Broker Activity Awaiting Review"; run a live Schwab scanner pass if OAuth is connected; verify Eric cannot see any of Matt's broker positions/imports/account data.
