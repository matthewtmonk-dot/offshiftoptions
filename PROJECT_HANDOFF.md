...
Last verified (August 31, 2026, against local Docker PostgreSQL and a fresh local Next dev server on port 3001 — nothing run against Supabase):
...
| Vitest unit tests | `corepack pnpm test` | 110 passed / 41 skipped (DB tests skip without `RUN_DB_TESTS`) |
| Vitest incl. DB integration | `RUN_DB_TESTS=1 DATABASE_URL=<local Docker Postgres URL> corepack pnpm test` | 151/151 passed across 29 files against local Docker Postgres; existing `pg` deprecation warning only |
| Playwright e2e | `PLAYWRIGHT_BASE_URL=http://127.0.0.1:3001 corepack pnpm exec playwright test` | 6/6 passed; includes mobile overflow sweeps across all main routes plus a manual end-to-end import/reconcile browser run (upload -> preview -> confirm -> link to Campaign), screenshotted and removed after verification |
...
- The current working tree contains the local Schwab credential-routing/CSV-import/reconciliation slice and migration `20260831235050_schwab_credential_routing_broker_records`. Do not push this slice until Matt applies that migration to production Supabase and verifies it.
