...
| Playwright e2e (existing regression) | `PLAYWRIGHT_BASE_URL=http://localhost:3001 corepack pnpm exec playwright test tests/e2e/phase1b.spec.ts` | 4/6 passed. 2 failures are an unrelated, pre-existing dev-mode-only artifact (see Known Issues — "Next.js dev overlay intercepts clicks in headless testing"), not a functional regression; the underlying sign-out mechanism was independently verified working (`form.requestSubmit()` bypassing the overlay). |
...
- **Next.js dev overlay intercepts clicks in headless testing (pre-existing, unrelated to appearance work)**: the CSP's `script-src` lacks `unsafe-eval` (see the existing CSP bullet above), which makes `next dev` show a persistent floating dev-tools indicator badge with a "1 Issue" warning. That badge sits bottom-left of the viewport, which happens to overlap the sidebar's Sign Out button, and its `<nextjs-portal>` intercepts the actual OS-level click coordinate — so Playwright (and a real developer) cannot click Sign Out there in dev mode; `force: true` doesn't help because the click still lands on the overlay, not the button underneath. Never appears in a production build (`next build && next start`) — dev-mode only. Not fixed in this slice (out of scope — CSP is a security control, not appearance); a real fix would be `devIndicators` repositioning in `next.config.ts` or resolving the underlying `unsafe-eval` gap, either of which is a separate, deliberate decision for Matt.
...
- **Application-wide Appearance System (Dark/Light/System) is complete and locally verified (2026-09-01), not yet pushed/deployed.** See Appearance System section for full detail. Requires migration `20260901031101_user_appearance_preference`, applied locally only.
...
- The scanner enhancement work, the Campaign Tracker foundation, the Schwab OAuth/market-data foundation, the Core Trading Cockpit + Account Ledger slice, and the Dashboard broker-position follow-up fix are **committed and pushed to `origin/main`**; production was previously live at commit `7972423`.
- The current working tree contains the local Schwab credential-routing/CSV-import/reconciliation slice (migration `20260831235050_schwab_credential_routing_broker_records`) **and** the Appearance System slice (migration `20260901031101_user_appearance_preference`). Do not push either until Matt applies its migration to production Supabase and verifies it. **Do not push at all until Matt explicitly approves — the Appearance System slice ends this session with a report only, per explicit instruction.**
...
**NOW**
- Matt to review and approve the Appearance System slice (report delivered at end of that session), then apply migration `20260901031101_user_appearance_preference` to production Supabase before pushing it.
