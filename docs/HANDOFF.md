...
## 2026-08-28 Pre-Schwab Production Hardening

With the app live on Hostinger/Supabase and Matt/Eric able to log in, this session hardened the release ahead of Schwab API approval. No Schwab work was started.

- **Confirmed clean build**: `package.json`'s `build` was already the permanent `prisma generate && next build` with no leftover bootstrap/migration steps from the deployment saga; `build:first-deploy` was already removed. Investigated the `EACCES` root cause further and documented a concrete migration runbook (manual/CI `prisma migrate deploy` against Supabase before pushing a schema-changing commit) in `docs/ARCHITECTURE.md`.
- **Rebranded** to "Off Shift Options" (short name "OSO") across the page title, login screen, nav/header, PWA manifest, install screen, metadata, and Open Graph tags. Replaced the placeholder SVG-only icon setup with real 192x192/512x512 PNGs (`any` and `maskable` purpose), a 180x180 Apple touch icon, and a rebranded favicon — generated once locally via the `sharp` package already present transitively through Next.js, not added as a project dependency. "My LST" scanner-profile terminology (the trading strategy name) was deliberately left alone.
- **Added a Change Password screen** (`/account`) — current-password verification, minimum length + letter/number complexity, `bcryptjs` hashing consistent with sign-in, and it now signs out every other session for that user while keeping the current one. Covered by a DB-backed integration test using disposable test users (never touches Matt/Eric's real local dev credentials).
- **Made chat live** with a small polling client component (`router.refresh()` every 4s while the tab is visible) instead of adding WebSockets/Supabase Realtime — the simplest option compatible with the existing Server Component + Server Action architecture.
- **Added visible DEMO/mock-data labeling** to the scanner page (banner + per-result badge) and the dashboard's scanner widget, since Schwab is still pending and results must not look like live data.
- **Security**: added standard production headers in `next.config.ts` (X-Frame-Options, X-Content-Type-Options, Referrer-Policy, Permissions-Policy, HSTS, a baseline CSP), stopped `/api/health` from returning the raw database error message on failure (logs server-side only now), and changed `/api/push-subscriptions` to return a clean 401 instead of a redirect for unauthenticated requests. Audited authorization, session/cookie handling, and tracked files for secrets — found no vulnerabilities and no committed secrets (only `.env.example` placeholders are tracked).
- **Production quality**: removed a stray empty `lst-buddy/` scaffolding directory and its `.dockerignore` entry, fixed a Playwright test that referenced the old "LST Buddy" branding text, and fixed a second Playwright test that wasn't idempotent against the shared local dev database (it used a fixed message string, so repeated local runs accumulated duplicates and eventually failed a strict-mode locator — now uses a timestamped message).
- **Verified locally**: `prisma validate`, `typecheck`, `lint`, unit tests, DB integration tests (`RUN_DB_TESTS=1` against local Docker Postgres), the full Playwright suite against a rebuilt Docker app container, and a real `next build && next start` (production mode, not `next dev`) smoke test confirming dev-only login buttons are hidden, security headers are present, and icons/manifest resolve correctly. Nothing was run against Supabase or Hostinger from this machine.
- **Deferred by design, not forgotten**: full Web Push delivery (VAPID keys, `web-push` dependency, client subscribe flow, service worker `push` handler) is documented in `docs/PWA_AND_NOTIFICATIONS.md` as a Phase 2/3 task — HTTPS is no longer the blocker, but the remaining work is a real scope expansion, not something to bolt on here.
